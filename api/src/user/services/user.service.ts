...
export class UserService {
    ...
    ////SHOULD BE TESTED
    async updateGameHistoryScore(id: number, gameID: number, score: number): Promise<void> {
        const user = await this.userRepository.findOneBy({ id: id });
        if (user) {
    ...
    async updateRanks(): Promise<void> {
        ...
        const usersToUpdate = users.filter((user) => user.score !== 1200);
        let index = 1;

        for (const user of usersToUpdate) {
            await this.userRepository.update(user.id, { rank: index });
            index++;
        }
