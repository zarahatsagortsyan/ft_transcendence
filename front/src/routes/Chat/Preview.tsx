...
//           setMenuEvent(null);
//         }
//       }, [blockedList, show, hide, current, menuEvent]);
...
//                 })
//                 }
//                 <Menu id={JSON.stringify(global.selectedChat)}>
...
//   onClick?: () => void;
//   selected: boolean;
//   blockedList: any[]; // Update the type of blockedList
//   setHide: (d: any) => void;
...
//       const result: undefined | string | Blob | MediaSource = await getUserAvatarQuery(data.ownerId);
...
//         onContextMenu={handleMenu} // Remove the arrow function from onContextMenu
//       >
//         <div>
//           <div
...
export default function Preview({
  ...
  if (global.selectedChat) {
    const blockedChat = blockedList.find((map: any) => map.id === global.selectedChat.ownerId);
    global.selectedChat.isBlocked = blockedChat ? blockedChat : undefined;
  }

//   const handleMenu = useHandleMenu(global.selectedChat, blockedList, setHide, setMenuEvent);
    ...
    <div className="preview-zone">
      <div className="preview-chat-search">
        <ChatSearch
          ...
          updateStatus={updateStatus}
        />
        <AddRoom onRequest={onNewRoomRequest} />
      </div>
      <div className="preview-chat-list">
        {roomPreview.map((value, index) => (
          <div key={index}>
...
function useHandleMenu(data: chatPreview, blockedList: any[], setHide: (hide: any) => void, setMenuEvent: (event: any) => void) {
  const { hideAll } = useContextMenu();
...
function ChatSearch({
  ...
  function PreviewChat({
    ...
    useEffect(() => {
      const getAvatar = async () => {
        const result: undefined | string | Blob | MediaSource = await getUserAvatarQuery(
          data.ownerId
        );
    ...
    const handleMenu = useHandleMenu(data, blockedList, setHide, setMenuEvent);
